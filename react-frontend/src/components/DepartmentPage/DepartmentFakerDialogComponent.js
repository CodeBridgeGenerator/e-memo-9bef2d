...
import departmentFakerFactory from "./departmentFakerFactory";
...
const DepartmentFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [deptidIds, setdeptidIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("users")
                        .find({ query: { $select: ["_id","nodept"] } })
                        .then((data) => {
                        setdeptidIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = departmentFakerFactory(props.user,fakerCount,deptidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("department").create(elem));
...
export default connect(mapState, mapDispatch)(DepartmentFakerDialogComponent);
