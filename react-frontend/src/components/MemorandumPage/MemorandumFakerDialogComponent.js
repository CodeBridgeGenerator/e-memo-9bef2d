...
import memorandumFakerFactory from "./memorandumFakerFactory";
...
const MemorandumFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [memoidIds, setmemoidIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("users")
                        .find({ query: { $select: ["_id","memoid"] } })
                        .then((data) => {
                        setmemoidIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = memorandumFakerFactory(props.user,fakerCount,memoidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memorandum").create(elem));
...
export default connect(mapState, mapDispatch)(MemorandumFakerDialogComponent);
